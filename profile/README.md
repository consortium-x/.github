<p align="center">
  <img src="https://github.com/consortium-x/.github/blob/main/documents/images/company-logos/CX-transparent.avif"
       alt="Consortium X Logo"
       width="200"
       height="200">
</p>

<p align="center">
  <img src="https://github.com/consortium-x/.github/blob/main/documents/images/badges/Bronze%20-%20Welsh%20Banner%20(2021).png"
       alt="Armed Forces Coveneant Bronze Banner"
       width="auto"
       height="200">
</p>

# Consortium X LTD

## About Us

Consortium X (CX) is a veteran-founded and led technology company, specialising in Complex Systems Engineering, from architecture and design through implementation, integration, and verification & validation. Headquartered in Wales and serving clients nationwide, we support organisations in delivering assured outcomes across complex, high-value, and strategically important sectors. Our expertise spans Software Engineering and secure Digital Solutions, as well as Engineering Project Management, with a particular depth in Mission Systems. Our team combines operational insight with industry-leading technical capabilities to deliver resilient, future-ready results.

## Our Expertise

Our capability spans:

- Complex Systems Engineering  
- Software Engineering and Secure Digital Solutions  
- Engineering Project Management  
- Mission Systems architecture, integration, and assurance  

We bring particular depth in **Mission Systems**, where operational experience is combined with industry-leading technical capability to deliver **resilient, future-ready systems**.

---

## How We Operate

- Delivery-led, engineer-driven approach  
- End-to-end accountability for technical outcomes  
- Strong emphasis on assurance, safety, and security  
- Experience operating within regulated and high-consequence environments  

Our work typically sits at the interface between platform, mission system, and digital infrastructure — where integration, clarity, and correctness matter.

---

## This GitHub Organisation

This space contains selected technical work, tooling, and supporting artefacts that reflect how we engineer and think.

Not all work can be public. What is published here is intentional.

---

**Veteran-founded | Wales-based | UK-focused**
<!--

**Here are some ideas to get you started:**

🙋‍♀️ A short introduction - what is your organization all about?
🌈 Contribution guidelines - how can the community get involved?
👩‍💻 Useful resources - where can the community find your docs? Is there anything else the community should know?
🍿 Fun facts - what does your team eat for breakfast?
🧙 Remember, you can do mighty things with the power of [Markdown](https://docs.github.com/github/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax)
-->
